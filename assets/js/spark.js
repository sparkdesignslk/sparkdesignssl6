...
function initThree(){
  ...
  function animate(){
    ...
    allPyramids.forEach((mesh,i)=>{
      mesh.rotation.x=t*(0.22+i*0.025)+(i%2?0.6:0);
      mesh.rotation.y=t*(0.36+i*0.035);
      mesh.rotation.z=Math.sin(t*0.28+i)*0.18;
    });
    ...
    keyLight.position.x=-3.5+Math.sin(t*0.8)*1.2;
    keyLight.position.y=2.2+Math.cos(t*0.7)*0.8;
    fillLight.position.x=4+Math.cos(t*0.65)*1.1;
    keyLight.intensity=4.1+Math.sin(t*1.4)*1.0;
    fillLight.intensity=2.8+Math.sin(t*1.2+1)*0.8;
...
function initLightning(){
  ...
  function randomBolt(){
    ...
    for(let i=0;i<segs;i++){
      const nx=cx+(Math.random()-0.5)*54+Math.cos(angle)*len/segs;
      const ny=cy+(Math.random()-0.5)*36+Math.sin(angle)*len/segs;
      branches.push({x1:cx,y1:cy,x2:nx,y2:ny});
      if(Math.random()<0.48){
        const bx=nx+(Math.random()-0.5)*90;
        const by=ny+Math.random()*75;
        branches.push({x1:nx,y1:ny,x2:bx,y2:by,branch:true});
      }
      cx=nx;cy=ny;
    }
    return{branches,life:1,decay:0.012+Math.random()*0.018,flicker:Math.random()<0.55};
  }

  // Spawn bolts randomly
  function maybeSpawn(){
    if(bolts.length<MAX_BOLTS&&Math.random()<0.42){
      bolts.push(randomBolt());
    }
    setTimeout(maybeSpawn,140+Math.random()*220);
  }
  for(let i=0;i<5;i++) bolts.push(randomBolt());
  maybeSpawn();
  ...
  function drawFrame(){
    ...
    for(let i=bolts.length-1;i>=0;i--){
      const bolt=bolts[i];
      if(bolt.flicker&&Math.random()<0.18){bolt.life-=0.004;}
      const alpha=bolt.life*(0.72+Math.random()*0.18);
      bolt.branches.forEach(seg=>{
        const w=seg.branch?1.1:2.8;
        ctx.beginPath();
        ...
        ctx.shadowBlur=seg.branch?10:24;
        ctx.stroke();
        // White hot core
        ctx.beginPath();
        ctx.moveTo(seg.x1,seg.y1);
