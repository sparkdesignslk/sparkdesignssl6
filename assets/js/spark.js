...
function boot() {
  ...
  initLightning();
  animHero();
  gsap.to('.s-img', { opacity: 0.42, duration: 1.2, stagger: 0.15, delay: 0.2, ease: 'power2.out' });
  document.querySelectorAll('.s-img').forEach(el => el.classList.add('show'));
  initScrollAnim();
}
...
window.addEventListener('scroll',()=>{
  ...
  if(sy<hero.offsetHeight){
    gsap.set('.hero-content',{y:sy*0.2});
...
function renderWheel(){
  slotEls.forEach((el,i)=>{
    ...
    el.style.transition='transform 0.45s cubic-bezier(.16,1,.3,1), opacity 0.45s, filter 0.45s';
  });
...
document.addEventListener('keydown',e=>{ if(!lightbox.classList.contains('open'))return; if(e.key==='Escape')closeAlbum(); if(e.key==='ArrowDown'||e.key==='ArrowRight'){albumIdx=(albumIdx+1)%albumImages.length;renderWheel();} if(e.key==='ArrowUp'||e.key==='ArrowLeft'){albumIdx=(albumIdx-1+albumImages.length)%albumImages.length;renderWheel();} });

// Wheel drag
let dragStart=null;
albumStage.addEventListener('mousedown',e=>{dragStart=e.clientY;});
albumStage.addEventListener('mousemove',e=>{ if(dragStart===null)return; if(Math.abs(e.clientY-dragStart)>40){ albumIdx=e.clientY<dragStart?(albumIdx+1)%albumImages.length:(albumIdx-1+albumImages.length)%albumImages.length; dragStart=e.clientY; renderWheel(); }});
...
albumStage.addEventListener('touchmove',e=>{ if(dragStart===null)return; if(Math.abs(e.touches[0].clientY-dragStart)>40){ albumIdx=e.touches[0].clientY<dragStart?(albumIdx+1)%albumImages.length:(albumIdx-1+albumImages.length)%albumImages.length; dragStart=e.touches[0].clientY; renderWheel(); }},{passive:true});
...
function initLightning(){
  ...
  function maybeSpawn(){
    if(bolts.length<MAX_BOLTS&&Math.random()<0.04){
  ...
  function drawFrame(){
    ...
    for(let i=bolts.length-1;i>=0;i--){
      ...
      bolt.branches.forEach(seg=>{
        ...
        ctx.strokeStyle=`rgba(220,0,0,${alpha*0.9})`;
        ctx.lineWidth=w;
        ctx.lineCap='round';
